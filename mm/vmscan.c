...
int sysctl_lru_gen = 1;
EXPORT_SYMBOL_GPL(sysctl_lru_gen);


#ifdef CONFIG_LRU_GEN
/* ligado por padrão; controlado via /proc/sys/vm/lru_gen */
...
static int __init mglru_init(void)
{
        /* Disabled by default */
        return 0;
}
subsys_initcall(mglru_init);
...
/* sysctl padrão já existe no topo? Se não, descomente estas duas linhas:
*/

int lru_gen_enabled(void)
{
        return READ_ONCE(sysctl_lru_gen);
}
EXPORT_SYMBOL_GPL(lru_gen_enabled);

void lru_gen_init_lruvec(struct lruvec *lruvec)
{
        struct lru_gen_struct *lrugen = &lruvec->lru_gen;
        int gen;
        ...
        for (gen = 0; gen < MAX_NR_GENS; gen++) {
                INIT_LIST_HEAD(&lrugen->lists[gen][LRU_GEN_ANON]);
                INIT_LIST_HEAD(&lrugen->lists[gen][LRU_GEN_FILE]);
        }
}
...
{
        struct lru_gen_struct *lrugen = &lruvec->lru_gen;
        enum lru_gen_type type = PageAnon(page) ? LRU_GEN_ANON : LRU_GEN_FILE;
        int gen;
        ...
        list_add_tail(&page->lru, &lrugen->lists[gen][type]);
...
void lru_gen_del_page(struct lruvec *lruvec, struct page *page, bool reclaim)
{
        ...
        spin_lock(&lruvec->lru_gen.lock);
        if (!list_empty(&page->lru))
                list_del_init(&page->lru);
        spin_unlock(&lruvec->lru_gen.lock);
}

/*
 * Aging mínimo: avança a sequência e "abre" novas cabeças de lista para anon/file.
 * Integração com reclaim mais profunda pode ser feita depois; isso já é funcional.
 */
void lru_gen_age_lruvec(struct lruvec *lruvec, struct scan_control *sc)
{
        ...
        /*
         * Estratégia simples: mantém min_seq não avançado aqui;
         * o shrink decide quando incrementar min_seq[*] ao esvaziar gerações.
         * (Podemos evoluir isso quando plugarmos no shrink_inactive_list)
         */
        spin_unlock(&lrugen->lock);
}
EXPORT_SYMBOL_GPL(lru_gen_age_lruvec);
